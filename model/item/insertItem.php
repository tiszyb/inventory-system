<?php
	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			// Check if item name already exist in db
			$Sql = 'SELECT itemName FROM item WHERE itemName=:itemName';
			$stockStatement = $conn->prepare($Sql);
			$stockStatement->execute(['itemName' => $itemName]);
			if($stockStatement->rowCount() > 0){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Item already exists in DB. Please click the <strong>Update</strong> button to update the details. Or use a different Item Name.</div>';
				exit();
			}

...
function roundto(num){
	var cur =  Math.round(num * 100) / 100;
	return cur.toLocaleString('en-NG', {
		style: 'currency',
		currency: 'NGN',
	});
}
	...
	$(tableContainerDivID).load(tableCreatorFileUrl, function(){
		...
		$(tableID).DataTable({
			...
			"footerCallback": function ( row, data, start, end, display ) {
				...
				$( api.column( 6 ).footer() ).html(quantityFilteredTotal +' <br> &darr; <br> ('+ quantityTotal +')');
				$( api.column( 7 ).footer() ).html(roundto(unitPriceFilteredTotal) +' <br> &darr; <br>('+ roundto(unitPriceTotal) +')');
				$( api.column( 8 ).footer() ).html(roundto(fullPriceFilteredTotal) +' <br> &darr; <br>('+ roundto(fullPriceTotal) +')');
			}
		});
	});
}
	...
	$(tableContainerDivID).load(tableCreatorFileUrl, function(){
		...
		$(tableID).DataTable({
			...
			"footerCallback": function ( row, data, start, end, display ) {
				...
				// Update footer columns				
				$( api.column( 7 ).footer() ).html(quantityFilteredTotal +' <br> &darr; <br> ('+ quantityTotal +')');
				$( api.column( 8 ).footer() ).html(roundto(unitPriceFilteredTotal) +' <br> &darr; <br>('+ roundto(unitPriceTotal) +')');
				$( api.column( 9 ).footer() ).html(roundto(fullPriceFilteredTotal) +' <br> &darr; <br>('+ roundto(fullPriceTotal) +')');
			}
		});
	});
}
...
function addCustomer() {
	...
	$.ajax({
		...
		complete: function(data){
			...
			searchTableCreator('customerDetailsTableDiv', customerDetailsSearchTableCreatorFile, 'customerDetailsTable');
			reportsTableCreator('customerReportsTableDiv', customerReportsSearchTableCreatorFile, 'customerReportsTable');
			setTimeout(()=>{$('form').find("input[type=text], input[type=number], textarea").val(""); $('.alert').hide();},1500)
		}
	});
}
...
function addPurchase() {
	...
	$.ajax({
		...
		success: function(data){
			...
			if( data == '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Purchase details added to database and stock values updated.</div>'){
				setTimeout(()=>{$('form').find("input[type=text], input[type=number], textarea").val(""); $('.alert').hide();},1500)
			}
		},
		complete: function(){
...
function addSale() {
	...
	$.ajax({
		...
		success: function(data){
			...
			if(data == '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Sale details added to DB and stocks updated.</div>'){
				setTimeout(()=>{$('form').find("input[type=text], input[type=number], textarea").val(""); $('.alert').hide();},1500)
			}
		},
		complete: function(){
